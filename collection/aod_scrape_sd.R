...
air_qualDir <- 'C:/Users/smdevine/Desktop/DSI_meetings/air_pollution_crop_yield_project/aerosol_rasters'
if (file.exists(file.path(air_qualDir, 'results')) == FALSE) {
  dir.create(file.path(air_qualDir, 'results'))
}
if (file.exists(file.path(air_qualDir, 'temp')) == FALSE) {
  dir.create(file.path(air_qualDir, 'temp'))
}
...
for (i in 1:length(datesequence)) { 
  doyr <- format.Date(datesequence[i], '%j')
  ...
  get_output <- GET(base_url2) #see ??? for guidelines on this because was not able to use the XML and RCurl packages to extract webpage links.  they only worked if I manually saved each webpage as a HTML file first
  urls <- readHTMLTable(rawToChar(get_output$content), stringsAsFactors = F)
  fname <- urls$`ftp-directory-list`[3,1]
  file_url <- paste0(base_url2, fname)
  setwd(file.path(air_qualDir, 'temp'))
  download.file(file_url, destfile = fname, mode = 'wb') #this fixed a problem https://gis.stackexchange.com/questions/213923/open-hdf4-files-using-gdal-on-windows
  ...
  gdal_translate(src_dataset = fname, dst_dataset = fname_tif, of = 'GTiff', sd_index = 12) #should print NULL on the screen
  ...
  if (i == 1) {
    results <- data.frame(date=datesequence[i], AOT_at_500nm=aod_value, data_layer=sbs[12], filename=fname, longitude=coordinates(res_station)[1], latitude=coordinates(res_station)[2])
